//crc.c
#include "main.h"
#include "stdint.h"

uint16_t crc16(uint8_t *adr_buffer, uint32_t byte_cnt)
{ 
	static unsigned char auchCRCHi[]=
    {
        ,
    };
    /*Table of CRC values for lowЦorder byte*/
		static char auchCRCLo[]=
    {
        ,
    };
    /////////////////////////////////////////////////////////////
		 // переменные дл¤ расчета CRC16 -?*:?????
//    unsigned char uchCRCHi = 0xFF;  
//    unsigned char uchCRCLo = 0xFF; 
uint8_t uchCRCHi = 0xFF;  
uint8_t uchCRCLo = 0xFF; 
unsigned uIndex; 
    
    /* CRC Generation Function */
    while( byte_cnt--) /* pass through message buffer */
    {
        uIndex = uchCRCHi ^ *adr_buffer++; /* calculate the CRC */
        uchCRCHi = uchCRCLo ^ auchCRCHi[uIndex];
        uchCRCLo = auchCRCLo[uIndex];
    }
return (uchCRCHi << 8 | uchCRCLo);
}
//uint16_t wd = ((uint16_t)d2 << 8) | d1;
uint16_t byte_to_word(uint8_t arr1,uint8_t arr2)
{
//uint8_t LL;
//uint8_t HH;
	return (((uint16_t)arr1<<8)|arr2);
}

uint8_t bit_to_byte(uint8_t byte, uint8_t bit)
{
if(bit==0){byte=1;}
else if (bit==1){byte=2;}
else if (bit==2){byte=4;}
else if (bit==3){byte=8;}
else if (bit==4){byte=16;}
else if (bit==5){byte=32;}
else if (bit==6){byte=64;}
else if (bit==7){byte=128;}
return byte;
}
